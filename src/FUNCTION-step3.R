step3 <- function(mvtrk, disp_outl, STBL_PRED = 7){
  ...
  disp_outl[is.na(disp_outl$phaseID),"phaseID"] <- paste0("N", disp_outl[is.na(disp_outl$phaseID),"clusterID"])
  ...
  for(i in 1:nrow(res_feature)){
    ...
    tmp_loc <- mvtrk %>% filter(Time <= res_feature$end_time[i] & Time >= res_feature$start_time[i]) %>% st_as_sf(coords = c("x","y")) %>% as_Spatial()
    res_feature$patch_size[i] <- kernel.area(kernelUD(tmp_loc), percent = c(95))
  ...
  for(i in 1:nrow(nres_feature)){
    ...
    if(nres_feature$Period[i] <= 0){break}
    tmp_loc <- mvtrk %>% filter(Time <= nres_feature$end_time[i] & Time >= nres_feature$start_time[i]) %>% st_as_sf(coords = c("x","y")) %>% mutate(step = NA)
    nres_feature$displacement[i] <- st_distance(tmp_loc[tmp_loc$Time==nres_feature$start_time[i],],tmp_loc[tmp_loc$Time==nres_feature$end_time[i],])
